...
class VideoLiveSmokeTest {
    ...
    @Test
    fun videoLive_emits_expected_events() {
        TestEventBus.clear()

        val scenario = ActivityScenario.launch(MainActivity::class.java)
        try {
            onView(withId(R.id.recyclerTestCases)).check(matches(isDisplayed()))
            clickOnTestCaseWithScroll(title = "Live Video")

            val expected = listOf(
                "VideoLive.onBuffering",
                "VideoLive.onReady",
                "VideoLive.onPlay",
                "VideoLive.playerViewReady",
            )

            val events = TestEventBus.awaitAll(expectedNames = expected, timeoutMs = 30_000, pollIntervalMs = 150)
