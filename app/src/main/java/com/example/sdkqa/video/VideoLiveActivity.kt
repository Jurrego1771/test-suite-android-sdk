    ...
    private fun createPlayerCallback(): MediastreamPlayerCallback {
        return object : MediastreamPlayerCallback {
            override fun playerViewReady(msplayerView: PlayerView?) {
                ...
                TestEventBus.record(
                    name = "VideoLive.playerViewReady",
            ...
            override fun onPlay() {
                Log.d(TAG, "onPlay")
                TestEventBus.record(name = "VideoLive.onPlay")
            }

            override fun onPause() {
                Log.d(TAG, "onPause")
                TestEventBus.record(name = "VideoLive.onPause")
            }

            override fun onReady() {
                Log.d(TAG, "onReady")
                TestEventBus.record(name = "VideoLive.onReady")
            }

            override fun onEnd() {
                Log.d(TAG, "onEnd")
                TestEventBus.record(name = "VideoLive.onEnd")
            }

            override fun onBuffering() {
                Log.d(TAG, "onBuffering")
                TestEventBus.record(name = "VideoLive.onBuffering")
            }

            override fun onError(error: String?) {
                Log.e(TAG, "onError: $error")
                TestEventBus.record(name = "VideoLive.onError", data = mapOf("error" to (error ?: "")))
            }
